...
export default function App() {
  ...
  useEffect(()=>{
    ...
    const {data:{subscription}} = supabase.auth.onAuthStateChange((_event,session)=>{
      if(session){
        ...
        setUser(null);setProfile(null);setDocs([]);setPage("login");
      }
    });
  ...
  const fetchDocs = async (uid) => {
    const {data,error} = await supabase.from("documents").select("*").eq("user_id",uid).order("created_at",{ascending:false});
    if(data) setDocs(data.map(d=>({...d,uploaded:d.uploaded_at,type:d.file_type,uploadedBy:d.uploaded_by})));
  };

  const fetchTemplates = async (uid) => {
    const {data} = await supabase.from("templates").select("*").eq("user_id",uid).order("created_at",{ascending:false});
    if(data) setSavedTemplates(data.map(t=>({...t,createdBy:t.created_by,createdAt:new Date(t.created_at).toLocaleDateString()})));
  };
  ...
  const processNextQueued = (docsState) => {
    ...
    queued.slice(0,slots).forEach(doc=>runExtractWithClaude(doc));
  };

  useEffect(()=>{
    const processing=docs.filter(d=>d.status==="Processing").length;
    const queued=docs.filter(d=>d.status==="Queued");
    if(processing<PROCESSING_SLOTS&&queued.length>0) processNextQueued(docs);
  ...
  const markReviewed = async (doc) => {
    ...
    await supabase.from("documents").update({status:"Reviewed",reviewer:reviewerName}).eq("id",doc.id);
    updateDoc(doc.id,{status:"Reviewed",reviewer:reviewerName});
    setReviewDoc(r=>r?{...r,status:"Reviewed",reviewer:reviewerName}:r);
    showToast("Marked as reviewed");
  };

  const saveTemplate = async (t) => {
    const {data} = await supabase.from("templates").insert({
      user_id:user.id,name:t.name,created_by:profile?.name||user.email,data:t.data
    }).select().single();
    if(data){
      setSavedTemplates(ts=>[{...data,createdBy:data.created_by,createdAt:new Date(data.created_at).toLocaleDateString()},...ts]);
...
function DashboardPage({docs,statusCounts,statusFilter,setStatusFilter,search,setSearch,selected,setSelected,onDelete,onDeleteBulk,onExport,onReview,dateFrom,setDateFrom,dateTo,setDateTo,sortCol,sortDir,onSort,currentPage,setCurrentPage,totalPages}){
  ...
  const Th=({col,label,center})=><th style={{padding:"10px 12px",textAlign:center?"center":"left",fontSize:12,color:COLORS.textMuted,fontWeight:500,cursor:"pointer",userSelect:"none",whiteSpace:"nowrap",borderBottom:`1px solid ${COLORS.border}`}} onClick={()=>onSort(col)}>{label}{sortCol===col?(sortDir==="asc"?" ↑":" ↓"):""}</th>;
  return(
    <div>
...
function UploadPage({queue,setQueue,onFileSelect,onBegin,templates}){
  ...
  return(
    <div style={{maxWidth:800}}>
      <div ref={dropRef} style={{...s.card,border:`2px dashed ${COLORS.border}`,textAlign:"center",padding:"3rem",cursor:"pointer"}} onDragOver={e=>{e.preventDefault();dropRef.current.style.borderColor=COLORS.accent;}} onDragLeave={()=>dropRef.current.style.borderColor=COLORS.border} onDrop={onDrop} onClick={()=>fileRef.current.click()}>
...
function ReviewPage({doc,onBack,onMarkReviewed,onExport,onRunClaude,user,profile,templates,onSaveTemplate}){
  ...
  const CB=({c})=>c?<span style={{background:confColor(c).bg,color:confColor(c).text,border:`1px solid ${confColor(c).border}`,padding:"1px 6px",borderRadius:20,fontSize:10,fontWeight:500,marginLeft:4,flexShrink:0}}>{c[0].toUpperCase()}</span>:null;
  ...
  const applyTemplate=t=>{setEditData(t.data);setShowApplyTemplate(false);};
      ...
      {showSaveTemplate&&(
        <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.6)",zIndex:50,display:"flex",alignItems:"center",justifyContent:"center"}} onClick={()=>setShowSaveTemplate(false)}>
          <div style={{...s.card,width:400,padding:"1.5rem"}} onClick={e=>e.stopPropagation()}>
            ...
            <div style={{fontSize:13,color:COLORS.textMuted,marginBottom:16,lineHeight:1.5}}>Save the current field structure as a reusable template to guide future extractions.</div>
            <label style={{fontSize:12,color:COLORS.textMuted,display:"block",marginBottom:6}}>Template Name</label>
            <input style={{...s.input,marginBottom:16}} value={templateName} onChange={e=>setTemplateName(e.target.value)} placeholder="e.g. Acme Supplier Invoice" autoFocus onKeyDown={e=>e.key==="Enter"&&doSaveTemplate()}/>
          ...
          <div style={{...s.card,width:480,padding:"1.5rem",maxHeight:"70vh",display:"flex",flexDirection:"column"}} onClick={e=>e.stopPropagation()}>
            ...
            <div style={{flex:1,overflowY:"auto"}}>
              {templates.length===0?(
                ...
                <div key={t.id} style={{...s.card,marginBottom:10,padding:"1rem",cursor:"pointer",display:"flex",alignItems:"center",gap:12}} onMouseEnter={e=>e.currentTarget.style.borderColor=COLORS.accent} onMouseLeave={e=>e.currentTarget.style.borderColor=COLORS.border} onClick={()=>applyTemplate(t)}>
                  <div style={{flex:1}}>
        ...
        <div style={{flex:"0 0 50%",borderRight:`1px solid ${COLORS.border}`,background:"#0d0f18",display:"flex",flexDirection:"column",overflow:"hidden"}}>
          ...
          {activeField&&(
            <div style={{padding:"8px 14px",background:"#1a1d27",borderBottom:`1px solid ${COLORS.border}`,fontSize:12,color:"#fbbf24",display:"flex",alignItems:"center",gap:8}}>
              <span>⬚</span><span>Draw a box to extract <strong style={{color:COLORS.text}}>{activeField.label}</strong></span>
            ...
            {(doc.pdfData||pdfDataOverride)?(
              ...
              <div style={{display:"flex",alignItems:"center",justifyContent:"center",height:"100%"}}>
                <div style={{width:"80%",maxWidth:360,background:COLORS.bgCard,border:`1px solid ${COLORS.border}`,borderRadius:8,padding:"2rem",textAlign:"center"}}>
                  ...
                  <div style={{fontSize:12,color:COLORS.textMuted,marginBottom:16}}>{doc.type}</div>
                  <div style={{fontSize:11,color:COLORS.textMuted,lineHeight:1.7,background:"#0d0f18",padding:"10px",borderRadius:6,marginBottom:12}}>
                    PDF preview is available when uploading directly. Click below to load a local copy for viewing.
                  </div>
                  <label style={{...s.btn(),display:"inline-block",cursor:"pointer",fontSize:12,padding:"6px 14px"}}>
                    Load PDF
                    <input type="file" accept=".pdf" style={{display:"none"}} onChange={e=>{const f=e.target.files[0];if(!f)return;const r=new FileReader();r.onload=ev=>setPdfDataOverride(ev.target.result);r.readAsArrayBuffer(f);}}/>
                  </label>
                </div>
              </div>
            )}
      ...
      {tab==="users"&&(
        <div style={{maxWidth:820}}>
          <div style={{...s.card,marginBottom:"1.5rem",padding:"1rem"}}>
            <div style={{fontSize:14,fontWeight:500,marginBottom:12}}>Invite User</div>
            <div style={{fontSize:13,color:COLORS.textMuted,marginBottom:12}}>Users sign up themselves via the login page. Use this to update roles and groups for existing users.</div>
          </div>
          <div style={s.card}>
            <table style={{width:"100%",borderCollapse:"collapse"}}>
              ...
              <tbody>
                {users.map(u=>(
                  <tr key={u.id} style={{borderBottom:`1px solid ${COLORS.border}`}}>
                    ...
                    <td style={{padding:"10px 12px"}}>
                      <input style={{...s.input,width:120}} value={u.group_name||""} onChange={e=>setUsers(us=>us.map(x=>x.id===u.id?{...x,group_name:e.target.value}:x))} onBlur={async e=>{await supabase.from("profiles").update({group_name:e.target.value}).eq("id",u.id);showToast("Group updated");}} placeholder="Group"/>
                    </td>
                    <td style={{padding:"10px 12px"}}>
                      <button style={{...s.btn("secondary"),padding:"4px 10px",fontSize:12}} onClick={async()=>{await supabase.from("profiles").update({active:!u.active}).eq("id",u.id);setUsers(us=>us.map(x=>x.id===u.id?{...x,active:!x.active}:x));showToast("Status updated");}}>
...
function SettingsPage({profile,setProfile,supabase,showToast}){
  ...
  return(
    <div style={{maxWidth:540,display:"flex",flexDirection:"column",gap:16}}>
      ...
      <div style={s.card}>
        <div style={{fontSize:15,fontWeight:600,marginBottom:4}}>How extraction works</div>
        <div style={{fontSize:13,color:COLORS.textMuted,lineHeight:1.8}}>
          ...
          <strong style={{color:COLORS.text,fontWeight:500}}>2. Extract</strong> — Claude reads the document via the secure server-side API proxy and returns all header and line fields with confidence scores.<br/>
          ...
          <strong style={{color:COLORS.text,fontWeight:500}}>4. Export</strong> — Download a structured CSV with header and all line items.<br/>
          <strong style={{color:COLORS.text,fontWeight:500}}>5. Templates</strong> — Save reviewed documents as templates to guide future extractions from the same supplier.
        </div>
      </div>
    </div>
  );
